...
##### 1. In this tutorial case, which variation of Observer Pattern that we use? Push model or Pull model?
In this tutorial case, we are using the Push model of the Observer Pattern. This is evident because the `NotificationService.notify` method actively sends notifications to all subscribers whenever an event such as product creation, deletion, or promotion occurs. The `update` method in the `Subscriber` struct directly receives the notification payload, meaning the publisher pushes data to subscribers without them needing to request it.

##### 2. What are the advantages and disadvantages of using Pull Model of Observer Pattern for this tutorial case?
If we used the Pull model instead of the Push model, subscribers would have to request updates from the publisher instead of getting them automatically. This could reduce unnecessary updates since subscribers only pull data when needed, giving them more control. However, this would also cause delays, as updates wouldn’t be received instantly, and could result in outdated information. Additionally, constantly checking for updates would make the system more complex and inefficient due to repeated API calls, making it less suitable for real-time notifications.

##### 3. What will happen to the program if we decide to not use multi-threading in the notification process?
Without multi-threading, the program would send notifications one by one, leading to delays, especially with many subscribers. Since each notification requires an `HTTP request`, the system would have to wait for each request to finish before moving to the next, making the process slow. This would reduce performance and make the system inefficient for handling a large number of subscribers. Multi-threading helps by sending notifications at the same time, keeping the system fast and responsive.
